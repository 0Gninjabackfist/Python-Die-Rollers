...
def roll(die,sides):  #define die rolling function, 1 parameters die (what are the die face values) and sides (how many sides does the die have)
    ...
    return (value, face+1) #the face here is if later i need to know which side of the die was roll (0-5)+1
...
for adie in attdice: #for each value in the attdice list we are going to do the below
    (aresult,aface) = roll(dice[adie],6) #we ask for the value from the 3 part roll function return, ignoring the 2nd and 3rd value
    attroll = attroll + int(aresult) #adding up the total of the die values
    if aresult == 0: intel = intel + 1
    faceslist.append((adie,aface))
print('Attacker total =', attroll,' with', intel, ' Intel and the faces were', faceslist) #print out the total of the values rolled
